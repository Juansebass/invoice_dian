...
class AccountInvoice(models.Model):
    ...
    report_products_inline_enabled = fields.Boolean(string='Product inline report')
    ...
    def _get_warn_pfx(self):
        ...
        if self.journal_id.is_einvoicing:
            ...
            today = datetime.strptime(
                format_datetime(self.with_context(use_babel=True).env, fields.Datetime.now(), tz=self.env.user.tz, dt_format='yyyy-MM-dd'), '%Y-%m-%d')
    ...
    def _post_dian(self, soft=True):
        ...
        """
        #res = super(AccountInvoice, self)._post_dian()
        res = {}
            ...
            if record.company_id.einvoicing_enabled and record.journal_id.is_einvoicing:
                ...
                if record.move_type in ("out_invoice", "out_refund"):
                    ...
                    if record.send_invoice_to_dian == '0':
                        ...
                        elif record.l10n_co_edi_type == '04':
    ...
    def action_invoice_sent(self):
        ...
        """
        self.ensure_one()
        template = self.env.ref('l10n_co_edi_ubl_dian.email_template_for_einvoice')
        buff = BytesIO()
            ...
            if origin_invoice.state in ('open', 'paid', 'posted'):
                for dian_document in origin_invoice.dian_document_lines:
                    if dian_document.state == 'done':
                        ...
                        billing_reference['CustomizationID'] = origin_invoice.l10n_co_edi_operation_type
                    else:
    ...
    def _get_active_dian_resolution(self):
        ...
        number_to = False
        technical_key = False
        resolution_number = self.journal_id.l10n_co_edi_dian_authorization_number
        technical_key = self.journal_id.technical_key
        date_from = self.journal_id.l10n_co_edi_dian_authorization_date
        date_to = self.journal_id.l10n_co_edi_dian_authorization_end_date
        number_from = self.journal_id.l10n_co_edi_min_range_number                      
        number_to = self.journal_id.l10n_co_edi_max_range_number

        if not resolution_number:
            raise UserError("You do not have an active dian resolution, contact with your administrator.")
    ...
    def _get_einvoicing_taxes(self):
        ...
        for tax in self.line_ids:
            if tax.tax_line_id.tax_group_id.is_einvoicing:
                if not tax.tax_line_id.l10n_co_edi_type:
                    raise UserError(msg1 % tax.name)

                tax_code = tax.tax_line_id.l10n_co_edi_type.code
                tax_name = tax.tax_line_id.l10n_co_edi_type.name
                retention = tax.tax_line_id.l10n_co_edi_type.retention
                rate = 1
                date = fields.Date.context_today(self)
                # tax_percent = '{:.2f}'.format(tax.tax_line_id.amount)
                tax_percent = str('{:.2f}'.format(tax.tax_line_id.amount))

                if retention and tax.tax_line_id.amount == 0:
                    raise UserError(msg2 % tax.name)
                elif not retention and tax.tax_line_id.amount < 0:
                    raise UserError(msg3 % tax.name)
                elif retention and tax.tax_line_id.amount < 0:
    ...
    def _get_tax_representative_party_values(self):
        ...
        return {
            'IDschemeID': supplier._get_vat_verification_code(),
    ...
    def _get_invoice_lines(self):
        ...
        msg3 = _("Your product: '%s', has no reference price, " +
                 "contact with your administrator.")
        msg4 = _("Your tax: '%s', has no e-invoicing tax type, " +
                 "contact with your administrator.")
        ...
        for invoice_line in self.invoice_line_ids.filtered(lambda x: not x.display_type):
            if not invoice_line.product_uom_id.l10n_co_edi_ubl:
            ...
            nota_ref = ''
            if self.l10n_co_edi_operation_type == '09':
                nota_ref = 'Contrato de servicios AIU por concepto de: ' + self.aiu

            invoice_lines[count] = {}
            invoice_lines[count]['Note'] = nota_ref or ''
            invoice_lines[count]['unitCode'] = invoice_line.product_uom_id.l10n_co_edi_ubl
            ...
            for tax in invoice_line.tax_ids:
                ...
                for tax_id in tax_ids:
                    if tax_id.tax_group_id.is_einvoicing:
                        if not tax_id.l10n_co_edi_type:
                            raise UserError(msg4 % tax.name)

                        retention = tax_id.l10n_co_edi_type.retention

                        if retention and tax_id.amount == 0:
                            raise UserError(msg5 % tax_id.name)
                        elif not retention and tax_id.amount < 0:
                            raise UserError(msg6 % tax_id.name)
                        elif not retention == 'tax' and tax_id.amount == 0:
                            pass

                        elif retention and tax_id.amount < 0:
                            invoice_lines[count]['WithholdingTaxesTotal'] = (
        ...
        #Condicional para hacer sumatorias en caso de que sea factura resumida
        if self.report_products_inline_enabled:
            original_return_lines = invoice_lines.copy()
            lines = self.invoice_line_ids.filtered(lambda x: not x.display_type)
            invoice_line = lines[0]
            invoice_lines = {}
            disc_amount = 0
            total_wo_disc = 0
            brand_name = False
            ...
            for line in lines:
                if line.price_subtotal != 0 and line.discount != 0:
                    disc_amount += (line.price_subtotal * line.discount) / 100

                if line.price_unit != 0 and line.quantity != 0:
                    total_wo_disc += line.price_unit * line.quantity
                

            invoice_lines[1] = {}
            invoice_lines[1]['Note'] = nota_ref or ''
            invoice_lines[1]['unitCode'] = invoice_line.product_uom_id.l10n_co_edi_ubl
            invoice_lines[1]['Quantity'] = 1
            invoice_lines[1]['PriceAmount'] = '{:.2f}'.format(sum([i.price_unit for i in lines]))
            invoice_lines[1]['LineExtensionAmount'] = '{:.2f}'.format(sum([i.price_subtotal for i in lines]))
            invoice_lines[1]['PricingReference'] = '{:.2f}'.format(sum([i.product_id.with_company(self.company_id).standard_price or 0.0 for i in lines]))
            invoice_lines[1]['MultiplierFactorNumeric'] = '{:.2f}'.format(sum([i.discount for i in lines]))
            invoice_lines[1]['AllowanceChargeAmount'] = '{:.2f}'.format(disc_amount)
            invoice_lines[1]['AllowanceChargeBaseAmount'] = '{:.2f}'.format(total_wo_disc)
            invoice_lines[1]['TaxesTotal'] = {}
            invoice_lines[1]['WithholdingTaxesTotal'] = {}
            invoice_lines[1]['SellersItemIdentification'] = invoice_line.product_id.default_code
            invoice_lines[1]['StandardItemIdentification'] = invoice_line.product_id.product_scheme_code or ''
            invoice_lines[1]['StandardschemeID'] = product_scheme_id.code or ''
            invoice_lines[1]['StandardschemeName'] = product_scheme_id.name or ''
            invoice_lines[1]['StandardschemeAgencyID'] = product_scheme_id.scheme_agency_id or ''
                            ...
                            if retention and tax_id.amount == 0:
                                ...
                                invoice_lines[1]['TaxesTotal'] = (
                                    line._get_invoice_lines_taxes(
                                        ...
                                        tax_id.amount,
                                        invoice_lines[1]['TaxesTotal']))
            if '01' not in invoice_lines[1]['TaxesTotal']:
                invoice_lines[1]['TaxesTotal']['01'] = {}
                invoice_lines[1]['TaxesTotal']['01']['total'] = 0
                invoice_lines[1]['TaxesTotal']['01']['name'] = 'IVA'
                invoice_lines[1]['TaxesTotal']['01']['taxes'] = {}
                invoice_lines[1]['TaxesTotal']['01']['taxes']['0.00'] = {}
                invoice_lines[1]['TaxesTotal']['01']['taxes']['0.00']['base'] = invoice_line.price_subtotal
                invoice_lines[1]['TaxesTotal']['01']['taxes']['0.00']['amount'] = 0
