...
class Applicant(models.Model):
    ...
    def create_employee_from_applicant(self):
        ...
        res["context"]["default_applicant_categ_ids"] = [(6, 0, self.categ_ids.ids)]
        res["context"]["default_applicant_type_id"] = self.type_id.id
        res["context"]["default_applicant_medium_id"] = self.medium_id.id
        res["context"]["default_applicant_job_id"] = self.job_id.id
        res["context"]["default_applicant_salary_proposed_extra"] = self.salary_proposed_extra
        res["context"]["default_applicant_salary_expected_extra"] = self.salary_expected_extra
        res["context"]["default_applicant_salary_proposed"] = self.salary_proposed
        res["context"]["default_applicant_salary_expected"] = self.salary_expected
        res["context"]["default_applicant_availability"] = self.availability
