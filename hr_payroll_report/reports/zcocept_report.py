...
class ZConceptReport(models.AbstractModel):
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        if minimal_layout:
            header = ''
            footer = self.sudo().env['ir.actions.report'].render_template("web.internal_layout", values=rcontext)
            spec_paperformat_args = {'data-report-margin-top': 10, 'data-report-header-spacing': 10}
            footer = self.sudo().env['ir.actions.report'].render_template("web.minimal_layout",
            ...
            header = self.sudo().env['ir.actions.report'].render_template("web.external_layout", values=rcontext)
            ...
            try:
                ...
                for node in root.xpath(match_klass.format('header')):
                    ...
                    headers = self.sudo().env['ir.actions.report'].render_template("web.minimal_layout",
        ...
        return self.sudo().env['ir.actions.report']._run_wkhtmltopdf(
            [body],
