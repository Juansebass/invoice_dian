...
class HrContract(models.Model):
    ...
    date_end_required = fields.Boolean(related='contract_type_id.date_end_required',
                                       tracking=True)
    arl_percentage = fields.Float('ARL Percentage', related='risk_type_id.percentage', digits=(32, 6), tracking=True)
    ...
    def action_send_email(self):
        ...
        for contract in self:
            ...
            contract.ensure_one()
            ir_model_data = contract.sudo().env['ir.model.data'] # check access JD
            try:
                template_id = \
                    ir_model_data.check_object_reference('hr_contract_completion', 'notice_letter_email_template')[1]
            except ValueError:
                template_id = False
            try:
