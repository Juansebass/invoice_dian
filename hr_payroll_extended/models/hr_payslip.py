...
class HrPayslip(models.Model):
    ...
    # payslip_total = fields.Float(compute="_compute_payslip_total", store=True)
    ...
    #     for payslip in self:
    #         payslip_total = sum([line.total for line in payslip.line_ids.filtered(
    #             lambda x: x.salary_rule_id.total_cost)])
    #         payslip.payslip_total = payslip_total / 160 if payslip_total else 0.0
    ...
    def control_payslip(self): #check revisar
            ...
            if self.contract_id.structure_type_id.default_schedule_pay == 'monthly':
                ...
                if not len(payslip) == 1:
                    raise exceptions.ValidationError(
                        _('The payroll cannot be confirmed as there is an unconfirmed payroll from the previous month.'))
    ...
    def action_send_email_mass(self, template_id, report_id, ):
        ...
        report = self.sudo().env['ir.actions.report'].search([('id', '=', report_id)])
