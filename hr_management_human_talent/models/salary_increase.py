...
TYPE_SALARY = [('sueldo_basico', 'SUELDO BÁSICO'),
    ...
    def charge_employee(self):
        ...
        if self.type_increase == 'ipc':
            ...
            contracts = self.env['hr.contract'].search(
                [('wage', '>=', self.min_amount), ('salary_type', '=', self.salary_type),
        ...
        if self.type_increase == 'minimum_wage':
            ...
            contracts = self.env['hr.contract'].search(
                [('wage', '<', min_wage.value), ('salary_type', '=', self.salary_type),
                 ('active', '=', True),
                 ('state', '=', 'open')])

            contracts_integral = self.env['hr.contract'].search(
                [('salary_type', '=', 'salario_integral'), ('wage', '<', integral_salary.value),
                 ('active', '=', True), ('state', '=', 'open')])

        if contracts or contracts_integral:
            self.increase_lines_ids.unlink()
            if contracts and self.salary_type == 'apoyo_sostenimiento' and self.type_aprentice_sena:
                contracts = contracts.filtered(lambda j: j.type_apprentice_sena == self.type_aprentice_sena)
            if contracts_integral and self.salary_type == 'apoyo_sostenimiento' and self.type_aprentice_sena:
    ...
    def increase_salary(self):
        if self.increase_lines_ids:
            ...
            if self.type_increase == 'minimum_wage':
                ...
                for line in self.increase_lines_ids:
                    increase = 0
                    if line.contract_id.salary_type == 'salario_integral':
...
class HrSalaryIncreaseLines(models.Model):
    ...
    salary_type = fields.Selection(selection=TYPE_SALARY,
                                                tracking=True,
                                                related='contract_id.salary_type')
