...
class RequestForNews(models.Model):
    ...
    new_organization_unit = fields.Many2one('hr.employee.unit', 'New organizational unit')
    ...
    current_cotizante_subtype_id = fields.Many2one('hr.employee.contributor.subtype', 'Current cotizante subtype')
    new_cotizante_subtype_id = fields.Many2one('hr.employee.contributor.subtype', 'New cotizante subtype')
    ...
    def contract_information(self):
        if self.employee_id:
            ...
            self.current_city_id = self.employee_id.req_city.id
            self.check_extend_number = False
            self.current_cotizante_subtype_id = self.employee_id.contributor_subtype_id.id if self.employee_id.contributor_subtype_id else False
            ...
            if self.model == 'contract':
                ...
                if contract:
                    ...
                    self.organization_unit_actual = contract.organizational_unit_id.name
                    ...
                    self.current_apprentice_type = contract.trainee_type if contract.trainee_type else False
                    self.end_date_stage_lective = contract.teaching_stage_end_date if contract.teaching_stage_end_date else False

                    self.current_extension_number = contract.number_extend + 1

                    type_salary = False
                    if contract.salary_type == 'sueldo_basico':
                        type_salary = 'basic_salary'

                    if contract.salary_type == 'salario_integral':
                        type_salary = 'integral_salary'

                    if contract.salary_type == 'apoyo_sostenimiento':
    ...
    def charge_entity(self): #return domain
        """
                ...
                if self.model == 'contract':
                    ...
                    if self.check_apprentice:
                        self.employee_id.update({
                            'arl_risk': self.risk_level,
                            'arl_partner_id': self.arl_id.id,
                        })
                        contract.update({
                            'trainee_type': self.new_apprentice_type,
                            'productive_stage_start_date': self.start_date_productive_stage,
                            'productive_stage_end_date': self.end_date_productive_stage,
                            'wage': self.new_salary
                        })

                    if self.check_cotizante_subtype:
                        self.employee_id.update({
                            'contributor_subtype_id': self.new_cotizante_subtype_id.id
                        })

                    if self.check_city:
                        contract.write({'req_city': self.new_city_id.id})
                        self.employee_id.write({'req_city': self.new_city_id.id})
                    ...
                    if self.check_change_organizational_unit:
                        new_contract['organizational_unit_id'] = self.new_organization_unit.id
                        self.employee_id.write({
                            'organizational_unit_id': self.new_organization_unit.id
                        })
